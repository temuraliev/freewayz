...
function parseAlbumImages(html: string, albumUrl: string): string[] {
  ...
  const urls: string[] = [];
  const imgRegex =
    /data-origin-src="([^"]+)"|data-original="([^"]+)"|src="([^"]+\.(?:jpg|jpeg|png|webp)[^"]*)"/gi;
  let m;
  while ((m = imgRegex.exec(html)) !== null) {
    const raw = m[1] || m[2] || m[3];
    if (!raw || raw.startsWith("data:") || raw.includes("avatar") || raw.includes("logo"))
      continue;
    const full = raw.startsWith("//")
      ? "https:" + raw
      : resolveUrl(raw, albumUrl);
    try {
      const norm = new URL(full);
      norm.search = "";
      if (!seen.has(norm.href)) {
        seen.add(norm.href);
        urls.push(full);
      }
    } catch {}
...
/**
 * GET /api/cron/check-suppliers
 *
 * Vercel Cron job: checks each active Yupoo supplier for new albums.
 * For each new album, sends an album of up to 5 photos to admin via Telegram
 * with Import/Close buttons.
 */
export async function GET(request: NextRequest) {
  ...
  if (!client) {
    return NextResponse.json({ error: "No Sanity token" }, { status: 500 });
  }
  ...
  if (!botToken || adminIds.length === 0) {
    return NextResponse.json({
      ok: true,
      skipped: "no bot token or admin ids",
    });
  }

  const suppliers = await client.fetch(
    `*[_type == "yupooSupplier" && isActive == true]{
      _id, name, url, lastAlbumCount, knownAlbumIds
    }`
  );
    ...
    try {
      ...
      for (const albumUrl of toNotify) {
        ...
        try {
          ...
          imageUrls = parseAlbumImages(html, albumUrl).slice(
            0,
            MAX_PREVIEW_IMAGES
          );
        } catch {}
        ...
        for (const adminChatId of adminIds) {
          try {
            ...
            await sendTelegramMessage(
              botToken,
              adminChatId,
              `${supplier.name}: ${albumTitle}\n${albumUrl}`,
              {
                inline_keyboard: [
                  [
                    {
                      text: "Импортировать",
                      callback_data: `import:${supplier._id}:${albumId}`,
                    },
                    {
                      text: "Закрыть",
                      callback_data: `close:${albumId}`,
                    },
                  ],
                ],
              }
            ...
            console.error(
              `Telegram notify error for ${adminChatId}:`,
              (e as Error).message
            );
          }
        }
      ...
      const allIds = albumUrls
        .map((u) => extractAlbumId(u))
      ...
      console.error(
        `Supplier check error (${supplier.name}):`,
        (e as Error).message
      );
    }
  }
